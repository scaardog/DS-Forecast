create or replace PROCEDURE GET_WEIGHT_WEAR_FORECAST (p_roll_type varchar2) AS 
-- declare
...
BEGIN
   ...
   -- i := 0;
    ...
    FOR r1 IN (SELECT DISTINCT rc.id_roll,rc.roll_supp
        ...
        SELECT '"prognosis": "'
                  || case 
                  when remainder_tn is null then -1 
                  else remainder_tn end
                  ||'",'
                  ||'"act_wear": "'
                  ||vs.wear_prc
                  ||'"}'
